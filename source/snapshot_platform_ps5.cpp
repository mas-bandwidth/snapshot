...
#if SNAPSHOT_PLATFORM == SNAPSHOT_PLATFORM_PS5
...
static void snapshot_randombytes_buf( void * const buf, const size_t size_const )
{
...
int snapshot_platform_init()
{
    ...
    {
        snapshot_printf( SNAPSHOT_LOG_LEVEL_WARN, "failed to load random sysmodule" );
        return SNAPSHOT_ERROR;
    }

    if ( randombytes_set_implementation( &snapshot_random_implementation ) != 0 )
    {
        snapshot_printf( SNAPSHOT_LOG_LEVEL_WARN, "failed to setup random bytes implementation" );
        return SNAPSHOT_ERROR;
    }

    if ( ( handle_net = sceNetPoolCreate( "net", HEAP_SIZE_NET, 0 ) ) < 0 )
    {
        snapshot_printf( SNAPSHOT_LOG_LEVEL_WARN, "failed to init network pool" );
        return SNAPSHOT_ERROR;
    }
...
{
    return SNAPSHOT_PLATFORM_PS5;
}

#else // #if SNAPSHOT_PLATFORM == SNAPSHOT_PLATFORM_PS5

int snapshot_ps5_dummy_symbol = 0;

#endif // #if SNAPSHOT_PLATFORM == SNAPSHOT_PLATFORM_PS5
